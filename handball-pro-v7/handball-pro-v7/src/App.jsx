...
// ═══════════════════════════════════════════════════
function RegisterPage({events,setEvents,matchStatus,setMatchStatus,matchInfo,onCloseMatch,onStartMatch}) {
  ...
  const [homeSide,setHomeSide]=useState("right"); // home ataca hacia la derecha por defecto
  const awaySide=homeSide==="right"?"left":"right"; // siempre opuesto
  ...
  const quickTap=(type,team)=>{
    ...
    setEvents(prev=>[...prev,{
      id:Date.now(),min:minute,team,type,
      attackSide:side,
      zone:null,quadrant:null,shooter:null,goalkeeper:null,sanctioned:null,
      completed:false,quickMode:true,
      ...score,
    }]);
  };
  ...
  const submitFull=()=>{
    ...
    if(completingId) {
      ...
      setEvents(prev=>prev.map(e=>e.id===completingId?{
        ...e,
        zone:form.zone,quadrant:form.quadrant,min,type:form.type,
        shooter:form.shooter?{name:form.shooter,number:fp.find(p=>p.name===form.shooter)?.number}:null,
        goalkeeper:form.goalkeeper?{name:form.goalkeeper,number:opponentTeam.players.find(p=>p.name===form.goalkeeper)?.number}:null,
        completed:true,
      }:e));
      setCompletingId(null);
    } else {
      setEvents(prev=>[...prev,{
        id:Date.now(),min,team:form.team,type:form.type,
        zone:form.zone,quadrant:form.quadrant,
        shooter:form.shooter?{name:form.shooter,number:fp.find(p=>p.name===form.shooter)?.number}:null,
        goalkeeper:form.goalkeeper?{name:form.goalkeeper,number:opponentTeam.players.find(p=>p.name===form.goalkeeper)?.number}:null,
        sanctioned:null,completed:true,
        ...score,
      }]);
    }
    setStep(null);
  ...
  const submitDisc=()=>{
    ...
    const currentTeam=form.team==="home"?homeTeam:awayTeam;
    setEvents(p=>[...p,{
      id:Date.now(),min,team:form.team,type:form.type,completed:true,
      sanctioned:form.sanctioned?{name:form.sanctioned,number:currentTeam.players.find(p=>p.name===form.sanctioned)?.number}:null,
  ...
  const FullMode=()=>{
    const [lForm,setLForm]=useState({type:"goal",team:"home",zone:null,quadrant:null,shooter:null,goalkeeper:null,sanctioned:null,minute:String(minute)});
    ...
    const hc=useMemo(()=>{const c={};events.filter(e=>e.zone).forEach(e=>{c[e.zone]=(c[e.zone]||0)+1;});return c;},[]);
    const lSubmit=()=>{
      const min=parseInt(lForm.minute)||1;
      const score=calcScore(lForm.type,lForm.team);
      const fp=currentTeam.players.filter(p=>p.pos!=="Arquero");
      setEvents(prev=>[...prev,{
        id:Date.now(),min,team:lForm.team,type:lForm.type,
      ...
      setLForm({type:"goal",team:"home",zone:null,quadrant:null,shooter:null,goalkeeper:null,sanctioned:null,minute:String(minute)});
    };
    const lSubmitDisc=()=>{
      const min=parseInt(lForm.minute)||1;
      const prev=events.filter(e=>e.hScore!=null).slice(-1)[0]||{hScore:0,aScore:0};
...
// ═══════════════════════════════════════════════════
function StatsPage({liveEvents}){
  // Derivar shots de eventos completos y rápidos por separado
  const fullShots = useMemo(()=>liveEvents.filter(e=>["goal","miss","saved"].includes(e.type)&&e.completed&&e.zone!=null).map(e=>({
  ...
  const quickShots = useMemo(()=>liveEvents.filter(e=>["goal","miss","saved"].includes(e.type)&&!e.completed).map(e=>({
    zone:null, result:e.type==="goal"?"goal":e.type==="saved"?"saved":"miss",
      ...
      {noData?(
        ...
        <>
          ...
          <div style={{display:"flex",gap:6,marginBottom:12}}>
            {(dataMode==="full"
              ? [{k:"court",l:"🏟 Cancha"},{k:"goal",l:"🥅 Arco"},{k:"players",l:"👥 Jugadores"}]
              : [{k:"players",l:"👥 Jugadores"}]
            ).map(t=>(
              <button key={t.k} onClick={()=>setMainTab(t.k)}
                style={{flex:1,background:mainTab===t.k?T.accent:T.card,color:mainTab===t.k?"#fff":T.muted,
                  border:`1px solid ${mainTab===t.k?T.accent:T.border}`,borderRadius:9,padding:"9px 4px",fontSize:11,fontWeight:700,cursor:"pointer"}}>
                {t.l}
              </button>
            ))}
          </div>
...
//  MATCHES PAGE
// ═══════════════════════════════════════════════════
function MatchesPage({matchStatus,liveMatchInfo,liveScore,completedMatches,showNewMatch,setShowNewMatch,showCloseConfirm,setShowCloseConfirm,setTab,setLiveMatchInfo,setLiveEvents,setMatchStatus,closeMatch,setEvoMatch,setShowEvo}) {
        ...
        return(
          <div key={m.id} style={{background:T.card,borderRadius:14,border:`1px solid ${T.border}`,padding:"12px 14px",marginBottom:8}}>
            ...
            <button onClick={()=>{setEvoMatch(m);setShowEvo(true);}}
              style={{width:"100%",background:T.card2,color:T.muted,border:`1px solid ${T.border}`,borderRadius:9,padding:"7px",fontSize:11,fontWeight:600,cursor:"pointer"}}>
              📊 Ver evolución →
            </button>
          </div>
        );
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.82)",backdropFilter:"blur(6px)",display:"flex",flexDirection:"column",justifyContent:"flex-end",zIndex:300}}>
          <div style={{background:T.card,borderRadius:"22px 22px 0 0",padding:"18px 18px 24px",border:`1px solid ${T.border}`}}>
            ...
            <div style={{display:"flex",gap:9}}>
              ...
              <button onClick={()=>{
                setLiveMatchInfo({home:newForm.home,away:newForm.away,date:null});
                setLiveEvents([]);
                setMatchStatus("live");
...
export default function App() {
  ...
  const [completedMatches,setCompletedMatches]=useState(DEMO_MATCHES);
  ...
  const closeMatch=()=>{
    const newMatch={
      id:Date.now(),
      home:liveMatchInfo.home, away:liveMatchInfo.away,
      hs:liveScore.h, as:liveScore.a,
      date:liveMatchInfo.date||new Date().toLocaleDateString("es-AR",{day:"2-digit",month:"2-digit"}),
  ...
  const content=()=>{
    if(showEvo) return <EvolutionPage match={evoMatch} goBack={()=>setShowEvo(false)}/>;
    switch(tab){
      case "matches": return <MatchesPage
        ...
        setTab={setTab} setLiveMatchInfo={setLiveMatchInfo} setLiveEvents={setLiveEvents}
    ...
    <div style={{background:T.bg,minHeight:"100vh",fontFamily:T.font,display:"flex",flexDirection:"column",alignItems:"center"}}>
      <div style={{width:"100%",maxWidth:430,minHeight:"100vh",background:T.bg,display:"flex",flexDirection:"column",position:"relative"}}>
        ...
        <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:"rgba(6,12,24,.97)",backdropFilter:"blur(16px)",borderTop:`1px solid ${T.border}`,display:"flex",zIndex:50,paddingBottom:"env(safe-area-inset-bottom,0)"}}>
          {NAV.map(n=>{
            const active=!showEvo&&tab===n.k;
            return(
              <button key={n.k} onClick={()=>{setShowEvo(false);setTab(n.k);}}
